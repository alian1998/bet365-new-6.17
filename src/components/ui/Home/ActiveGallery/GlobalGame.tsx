...
const GlobalGame = ({ activeCard, gamingData }: any) => {
  ...
  return (
    <div
      className={`grid ${
        activeCard == "365" || activeCard == "Sports"
          ? "grid-cols-2 gap-2"
          : "grid-cols-3 gap-2"
      }`}
    >
      {filter?.gameItems?.map((item: any) => (
        ...
        >
          <div
            className={`cardAmination ${
              activeCard == "365" || activeCard == "Sports" ? "p-1" : ""
            }  cursor-pointer`}
          >
            <div
              className={`relative w-full h-[110px] overflow-hidden rounded-md ${
                activeCard == "365" || activeCard == "Sports" ? "" : "p-1"
              }`}
            >
              <Image
