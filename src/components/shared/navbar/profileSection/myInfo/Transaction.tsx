...
const Transaction = ({
  ...
  useEffect(() => {
    fetchTransactions(7); // default 7 days
  }, []);

  const fetchTransactions = async (days: number) => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/transferTransaction?days=${days}`);
  ...
  const handleRefresh = () => {
    ...
    fetchTransactions(parseInt(filterValue));
  };
        ...
        <div className="cardColor2 mx-3 rounded flex items-center mt-5 justify-between p-2">
          <select
            ...
            onChange={(e) => {
              ...
              fetchTransactions(parseInt(newVal));
            }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="cardColor2 text-white sticky top-0">
                  ...
                  <th className="text-center px-2 py-2 font-normal">Txn Amount</th>
                  <th className="text-center px-2 py-2 font-normal">Before</th>
                  <th className="text-center px-2 py-2 font-normal">After</th>
                ...
                {transactions.length > 0 ? (
                  transactions.map((txn, index) => (
                    <tr key={index} className="allCards borderColor">
                      ...
                      <td className="px-2 py-2 text-center text-white1">{txn.afterBalance}</td>
                      <td className="px-2 py-2 text-right">
                        <span
  className={`font-medium ${
    txn.status === '-Debited' ? 'text-red-500' : 'text-[#0DBC84]'
  }`}
>
  {txn.status}
</span>
                  ...
                  <tr>
                    <td colSpan={5} className="text-white1 text-center py-4">
